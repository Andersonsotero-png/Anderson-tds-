...
/* util: mapar tipo->valor */
function valorParaTipo(tipo){
  if (tipo === 'inteira') return 35.90;
  if (tipo === 'meia') return 17.95;
  return 0;
}
...
if (form) {
  form.addEventListener('submit', e => {
    ...
    const tipoPulseira = (form.elements['tipoPulseira'] && form.elements['tipoPulseira'].value) || 'cortesia';
    const valorPulseira = valorParaTipo(tipoPulseira);
    ...
    if (idEmEdicao) {
      ...
      cadastros[idx] = {
        ...
        pulseira: { tipo: tipoPulseira, valor: valorPulseira }
      };
      saveCadastros();
      idEmEdicao = null;
...
function generateQRCodeCanvas(id){
  if(!qrDiv) return;
  qrDiv.innerHTML = '';
  // usamos o id simples como payload (para buscar por id)
...
if (inputBusca) inputBusca.addEventListener('input', () => {
  ...
  results.forEach(c => {
    ...
    const pulseiraText = c.pulseira ? `${(c.pulseira.tipo||'')} — ${money(c.pulseira.valor)}` : '—';
    li.innerHTML = `<div style="display:flex;justify-content:space-between;align-items:center">
      <div><strong>${escapeHtml(c.nome)}</strong> <small>(${escapeHtml(c.idade)} anos)</small><br>
        ...
        <div style="margin-top:6px"><small>Pulseira: ${escapeHtml(pulseiraText)}</small></div>
      </div>
...
function renderHistorico(){
  ...
  cadastros.forEach(c => {
    ...
    const badgeHTML = `<span class="badge ${getBadgeClass(c)}">${getBadgeText(c)}</span>`;
    const pulseiraText = c.pulseira ? `${escapeHtml(c.pulseira.tipo)} • ${money(c.pulseira.valor)}` : '—';
    div.innerHTML = `<strong>${escapeHtml(c.nome)}</strong> <small>${escapeHtml(c.idade)} anos</small>
      ...
      <div style="margin-top:6px"><strong>Pulseira:</strong> ${pulseiraText}</div>
...
function imprimirFicha(id){
  ...
  const html = `<html><head><meta charset="utf-8"><title>Ficha - ${escapeHtml(c.nome)}</title>
      ...
      <div>Setor: ${escapeHtml(c.setor||'-')} | Mesa: ${escapeHtml(c.mesa||'-')}</div>
      <div>Pulseira: ${(c.pulseira) ? escapeHtml(c.pulseira.tipo) + ' • ' + money(c.pulseira.valor) : '-'}</div>
      <div>Alergia: ${(c.temAlergia === 'sim') ? (escapeHtml(c.qualAlergia) || 'Sim') : 'Não'}</div>
      <div>Responsável: ${escapeHtml(c.responsavel || '-') } | Tel: ${escapeHtml(c.telefone || '-')}</div>
      <hr>
...
if (btnImprimir) btnImprimir.addEventListener('click', () => {
  ...
  let html = `<html><head><meta charset="utf-8"><title>Cadastros</title>
    ...
    <h2>Lista de Cadastros</h2><table><thead><tr><th>Nome</th><th>Idade</th><th>Tel</th><th>Email</th><th>Setor/Mesa</th><th>Status</th><th>Pulseira</th></tr></thead><tbody>`;
  cadastros.forEach(c => {
    html += `<tr><td>${escapeHtml(c.nome)}</td><td>${escapeHtml(c.idade)}</td><td>${escapeHtml(c.telefone||'-')}</td><td>${escapeHtml(c.email||'-')}</td>
      <td>${escapeHtml(c.setor||'-')} / ${escapeHtml(c.mesa||'-')}</td><td>${c.status==='dentro' ? 'No parque' : 'Fora do parque'}</td><td>${c.pulseira ? escapeHtml(c.pulseira.tipo)+' • '+money(c.pulseira.valor) : '-'}</td></tr>`;
  });
...
if (btnExportJSON) btnExportJSON.addEventListener('click', () => {
  ...
  // preparar dados (podemos limpar campos pesados)
  const dataForExport = cadastros.map(c => ({
    ...
    pulseira_tipo: c.pulseira ? c.pulseira.tipo : '',
    pulseira_valor: c.pulseira ? c.pulseira.valor : 0,
    status: c.status,
    createdAt: c.createdAt
  }));
  // criar worksheet & workbook
...
(function init(){
  cadastros = cadastros.map(c => ({
    ...
    pulseira: c.pulseira || { tipo: 'cortesia', valor: 0 },
    ...c
  }));
  renderHistorico();
  renderMarketingList();
...
function buildReportHTML(list, periodLabel){
  ...
  // total bruto
  const totalBruto = list.reduce((s,c) => s + ((c.pulseira && Number(c.pulseira.valor)) || 0), 0);
  ...
  let html = `<div id="relatorioPrint" class="report-wrapper">
    ...
    <table class="report-table" style="margin-top:12px">
      <thead><tr>
        <th>Nome</th><th>Idade</th><th>Setor</th><th>Tipo Pulseira</th><th>Valor</th>
      </tr></thead>
      <tbody>`;

  list.forEach(c => {
    const tipo = c.pulseira ? (c.pulseira.tipo || '-') : '-';
    const valor = c.pulseira ? (Number(c.pulseira.valor) || 0) : 0;
    html += `<tr>
  ...
  html += `</tbody></table>
    <div style="margin-top:12px;font-weight:700">Total bruto do período: ${money(totalBruto)}</div>
    </div>`;
  return html;
}
...
function abrirEdicao(id){
  ...
  try {
    if (form.elements['tipoPulseira']) form.elements['tipoPulseira'].value = (c.pulseira && c.pulseira.tipo) ? c.pulseira.tipo : 'cortesia';
